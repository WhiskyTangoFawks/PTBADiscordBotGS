...
public abstract class AbstractMoveParser {
    ...
    public MoveBuilder parseMove(RangeWrapper sheet, int i, int j) {
        ...
        if (sheet.getNote(i, j) != null && !sheet.getNote(i, j).isBlank()) {
            ...
            for (String parameter : note.split(";")) {
                ...
                if (tag.equalsIgnoreCase(SheetReaderService.PARSER.playbook_move.name()) || tag.equalsIgnoreCase(SheetReaderService.PARSER.basic_move.name())) {
    ...
    public MoveBuilder parseMoveFromNote(String note) {
        ...
        for (String parameter : note.split(";")) {
            ...
            if (split[0].equalsIgnoreCase(SheetReaderService.PARSER.playbook_move.name()) || split[0].equalsIgnoreCase(SheetReaderService.PARSER.basic_move.name())) {
                name = split[1];
